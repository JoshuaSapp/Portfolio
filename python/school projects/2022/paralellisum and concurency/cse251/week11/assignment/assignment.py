...
def main():
    ...
    for staff in range(CLEANING_STAFF):
        id_number = staff+1
        process_list.append(mp.Process(target=cleaner, args=(id_number,lock)))

    for visitor in range(HOTEL_GUESTS):
        id_number = visitor+1
        process_list.append(mp.Process(target=guest, args=(id_number,)))
