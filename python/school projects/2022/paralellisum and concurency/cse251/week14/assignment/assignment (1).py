...
def depth_fs_pedigree(family_id, tree):
    ...
    obj_family = Family(family_id, family.response)
    print("\nChildren")
    print(obj_family.children)

    if not tree.does_person_exist(obj_family.husband):
        #Requesting an individual from the server:
        person = Request_thread(f'{TOP_API_URL}/person/{obj_family.husband}')
        person.run()
        obj_person = Person(person.response)
        tree.add_person(obj_person)

    if not tree.does_person_exist(obj_family.wife):
        #Requesting an individual from the server:
        person = Request_thread(f'{TOP_API_URL}/person/{obj_family.wife}')
        person.run()
        obj_person = Person(person.response)
        tree.add_person(obj_person)

    for i in obj_family.children:
        if not tree.does_person_exist(i):
            #Requesting an individual from the server:
            person = Request_thread(f'{TOP_API_URL}/person/{i}')
            person.run()
    ...
    wife = tree.get_person(obj_family.wife)
    print("\nWife")
    print(wife)
    parent = Request_thread(f'{TOP_API_URL}/person/{wife.parents}')
    parent.run()
    print("\nParent")
    print(parent.response)

    # if(obj_family.husband != None):
    #     husband = threading.Thread(target = depth_fs_pedigree, args = (obj_family.husband, tree))
    #     husband.start()
    #     husband.join()

    # if(obj_family.wife != None):
    #     wife = threading.Thread(target = depth_fs_pedigree, args = (obj_family.wife, tree))
    #     wife.start()
    #     wife.join()
