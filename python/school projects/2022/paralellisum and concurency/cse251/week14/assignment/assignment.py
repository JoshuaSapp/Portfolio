...
def depth_fs_pedigree(family_id, tree):
    ...
    family_request.join()
    family_data = family_request.response

    # add family data to the tree
    family = Family(family_id,family_data)
    tree.add_family(family)

    # gather IDs of all family members
    husband_id = family_data['husband_id']
    wife_id = family_data['wife_id']
    children = family_data['children']


    # add children of the family to the tree
    for child_id in children:
        if not tree.does_person_exist(child_id):
            child_request = Request_thread(f'{TOP_API_URL}/person/{child_id}')
            child_request.start()
            child_request.join()
            child_data = child_request.response
            child = Person(child_data)
            tree.add_person(child)

    # add husband of the family to the tree
    if not tree.does_person_exist(husband_id) and husband_id != None:
        husband_request = Request_thread(f'{TOP_API_URL}/person/{husband_id}')
        husband_request.start()
        husband_request.join()
        husband_data = husband_request.response
        husband = Person(husband_data)
        tree.add_person(husband)   
    
    # add wife of the family to the tree
    if not tree.does_person_exist(wife_id) and wife_id != None:
        wife_request = Request_thread(f'{TOP_API_URL}/person/{wife_id}')
        wife_request.start()
        wife_request.join()
        wife_data = wife_request.response
        wife = Person(wife_data)
        tree.add_person(wife)    

    # If there is a husband, check their side of the tree
    if husband_id != None:
        husband_side_search = threading.Thread(target=depth_fs_pedigree,args=(husband_data['parent_id'],tree))
        husband_side_search.start()


    # If there is a wife, check their side of the tree
    if wife_id != None:
        wife_side_search = threading.Thread(target=depth_fs_pedigree,args=(wife_data['parent_id'],tree))
        wife_side_search.start()


    # after checking, rejoin the previous thread
    if husband_id != None:
        husband_side_search.join()
    if wife_id != None:
        wife_side_search.join()
