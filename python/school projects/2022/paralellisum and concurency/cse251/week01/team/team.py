...
if __name__ == '__main__':
    ...
    chunk = range_count/thread_count

    for i in range(thread_count):
        thread_start = start + (chunk*i)
        finish = start+(chunk*(i+1))
        threads.append(threading.Thread(target=check_thread,args=(thread_start,finish,)))
        threads[i].start()
