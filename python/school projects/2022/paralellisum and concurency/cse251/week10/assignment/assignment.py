...
class Writer(mp.Process):
  def __init__(self, shared, sem,items_to_send):
    mp.Process.__init__(self)
    self.items = items_to_send
    self.shared_data = shared
    self.sem = sem
    self.checkValue = BUFFER_SIZE
    self.totalCount = BUFFER_SIZE + 1
    self.activeIndex = BUFFER_SIZE + 2

  def run(self,):
    while self.shared_data[self.totalCount] != self.items:
      self.add_item()
        
  def add_item(self,):
    self.sem.acquire()
    check_value = self.shared_data[self.checkValue]
    active_index = self.shared_data[self.activeIndex]
    self.shared_data[active_index] = check_value+1
    
    
    

class Reader(mp.Process):
  def __init__(self, shared,sem):
    mp.Process.__init__(self)
    self.shared_data = shared
    self.sem = sem
    self.checkValue = BUFFER_SIZE
  ...
  def run(self):
    while self.shared_data[self.totalCount] != self.items_read:
      self.sem.release()
      print(self.shared_data.pop(0))
...
def main():
    ...
    sharable = smm.ShareableList(range(items_to_send))
    semi = mp.Semaphore(BUFFER_SIZE)
    ...
    for writer in range(WRITERS):
      processes.append(Writer(sharable,semi,items_to_send))

    for reader in range(READERS):
      processes.append(Reader(sharable,semi))
    ...
    print(f'{smm.index(BUFFER_SIZE + 1)} values received')
