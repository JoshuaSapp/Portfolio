...
class LinkedList:
    ...
    def remove(self, value):
        ...
        if current.next == None: # if the node is the tail of the list
            current.prev.next = None
            self.tail = current.prev
            return
        if current.prev == None: # if the node is the head of the list
    ...
    def __reversed__(self):
        ...
        """
        curr = self.tail  # Start at the end since this is a backwards iteration.
        while curr is not None:
            yield curr.data  # Provide (yield) each item to the user
