...
class Translator:
    ...
    def add_word(self, from_word, to_word):
        ...
        """
        self.words.update({f"{from_word}":f"{to_word}"})
    ...
    def translate(self, from_word):
        ...
        """
        return(self.words.get(f"{from_word}","???"))
    ...
    with open(filename) as file_in:
        for line in file_in:
            ...
            if fields[3] not in degrees:
                degrees.update({f"{fields[3]}":1})
            else:
                count = degrees.get(fields[3])
                degrees.update({f"{fields[3]}": count + 1})
...
def is_anagram(word1, word2):
    ...
    for letter in word_1:
        if letter != ' ':
            if letter not in d_word_1:
                d_word_1.update({f"{letter}": 1})
            else:
                count = d_word_1.get(letter)
                d_word_1.update({f"{letter}": count + 1})

    for letter in word_2:
        if letter != ' ':
            if letter not in d_word_2:
                d_word_2.update({f"{letter}": 1})
            else:
                count = d_word_2.get(letter)
                d_word_2.update({f"{letter}": count + 1})
    
    for index in d_word_1:
        if d_word_1.get(index) != d_word_2.get(index):
            match = False
...
def earthquake_daily_summary():
    ...
    for entry in features:
        ...
        print(f"{entry_properties.get('place')} - Mag {entry_properties.get('mag')}")
